...
function testMoveLogic() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    if (!effectiveStatus && mailData.filePath) {
      ...
      if (mailData.filePath.includes('\\rep\\') || mailData.filePath.includes('/rep/')) {
        effectiveStatus = "rep";
      } else if (mailData.filePath.includes('\\daRep\\') || mailData.filePath.includes('/daRep/')) {
        effectiveStatus = "daRep";
      } else if (mailData.filePath.includes('\\mustRep\\') || mailData.filePath.includes('/mustRep/')) {
        effectiveStatus = "mustRep";
      } else if (mailData.filePath.includes('\\chuaRep\\') || mailData.filePath.includes('/chuaRep/')) {
    ...
    const passed = actual.folder === testCase.expected.folder && actual.replied === testCase.expected.replied;
    
    console.log(`  Original status: ${mailData.status || 'undefined'}`);
    console.log(`  Effective status: ${effectiveStatus} (${mailData.status ? 'from field' : 'inferred from path'})`);
    console.log(`  Expected: ${testCase.expected.folder}, replied: ${testCase.expected.replied}`);
