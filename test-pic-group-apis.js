...
async function testPicGroupAPIs() {
  ...
  try {
    ...
    console.log('PICs:', Array.isArray(picsData) ? picsData.slice(0, 2) : 'No PICs found');
    ...
    console.log('Groups:', Array.isArray(groupsData) ? groupsData.slice(0, 2) : 'No Groups found');

    if (!Array.isArray(picsData) || !Array.isArray(groupsData) || !picsData.length || !groupsData.length) {
    ...
    const addResponse = await fetch(`${API_BASE_URL}/api/pics/${testPic.id}/groups`, {
      method: 'POST',
      headers: {
    ...
    const groupPicsResponse = await fetch(`${API_BASE_URL}/api/groups/${testGroup.id}/pics`);
    ...
    const removeResponse = await fetch(`${API_BASE_URL}/api/pics/${testPic.id}/groups/${testGroup.id}`, {
      method: 'DELETE',
    });
    ...
    const verifyResponse = await fetch(`${API_BASE_URL}/api/groups/${testGroup.id}/pics`);
