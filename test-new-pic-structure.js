...
async function testCreateNewPic() {
  ...
  try {
    ...
    if (createResult.success) {
      ...
      console.log('- Has isLeader field?', 'isLeader' in createResult.pic ? 'YES (❌)' : 'NO (✅)');

      // Test update PIC
      console.log('\n2️⃣ Testing update PIC...');
      const updateResponse = await fetch(`${API_BASE_URL}/api/pics/${createResult.pic.id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (updateResult.success) {
        ...
        console.log('- GroupLeaderships preserved:', updateResult.pic.groupLeaderships);
        console.log('- Has isLeader field?', 'isLeader' in updateResult.pic ? 'YES (❌)' : 'NO (✅)');
      }

      // Clean up - delete test PIC
      console.log('\n3️⃣ Cleaning up test PIC...');
      const deleteResponse = await fetch(`${API_BASE_URL}/api/pics/${createResult.pic.id}`, {
        method: 'DELETE',
      });
      const deleteResult = await deleteResponse.json();
      console.log('Delete result:', deleteResult.success ? '✅ Cleaned up' : '❌ Failed to clean up');
    }
